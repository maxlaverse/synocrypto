...
func TestEncryptingWithPublicKey(t *testing.T) {
	testCases := []string{"6C1FD4FA9566048ACE57BE85FC600ED914799F2A1AD31212D6678D30AC015D22"}

	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			encryptedSessionKey, err := EncryptOnceWithPublicKey([]byte(testdata.FixturePublicKey), []byte(tc))
			assert.NoError(t, err)

			sessionKey, err := DecryptOnceWithPrivateKey([]byte(testdata.FixturePrivateKey), []byte(encryptedSessionKey))
			assert.NoError(t, err)

			assert.Equal(t, string(sessionKey), tc)
		})
	}
}
